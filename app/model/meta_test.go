...
import (
	...
	"github.com/feisuweb/pogo/app/helper"
		...
		for t, f := range ShouldMetaFiles() {
			...
			if t == FormatTOML {
				Convey("ParseMetaToml", func() {
					...
					So(meta.Meta.Title, ShouldEqual, "PoGo")
					...
					Convey("NavGroup", func() {
						...
						So(meta.NavGroup[3].TrLink(i18n), ShouldEqual, "https://github.com/feisuweb/pogo")
					})
			...
			if t == FormatINI {
				Convey("ParseMetaIni", func() {
					...
					So(meta.Meta.DomainURL("/abc.html"), ShouldEqual, "http://pogo.io/docs/abc.html")
				})
			}
		}
	})
